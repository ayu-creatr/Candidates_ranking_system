...
# ── v7: Ownership and Evaluation Signals ───────────────────────────────────────
OWNERSHIP_SIGNALS = [
...
COMPANY_SCORES = {
    ...
    # Premium Global / Product tech companies (v7)
...
def compute_ownership_score(candidate: dict) -> float:
    """Computes ownership score in [0, 1] based on ownership signals in summary/experience."""
...
def compute_eval_score(candidate: dict) -> float:
    """Computes evaluation score in [0, 1] based on evaluation framework signals."""
    texts = [candidate["profile"].get("summary", "")]
    texts += [r.get("description", "") for r in candidate.get("career_history", [])]
    full = " ".join(texts).lower()
...
def generate_reasoning(candidate: dict, scores: dict) -> str:
    ...
    # Research low production penalty warning
...
def score_candidate(candidate: dict) -> tuple[float, str]:
    ...
    eval_val   = compute_eval_score(candidate)
    ...
    reasoning = generate_reasoning(candidate, {
        ...
        "ownership": ownership, "evaluation": eval_val,
...
def main():
    ...
    # Sort by score descending, break ties by candidate_id ascending (v7)
